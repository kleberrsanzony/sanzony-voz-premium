        ...
        <div className="container-site">
          <Reveal>
            <div className="text-center mb-20">
              <span className="section-label block mb-6">{t.sections.testimonials.label}</span>
          ...
          <StaggerGroup className="grid md:grid-cols-3 gap-6" staggerDelay={0.1}>
            {t.sections.testimonials.items.map((test, i) => (
              <StaggerItem key={i} className="luxury-card glass-card flex flex-col justify-between h-full group">
                ...
                <div className="pt-6 border-t border-white/10 group-hover:border-[#e0c27a]/20 transition-colors relative z-10">
                  <p className="font-display font-semibold text-sm text-[#e0c27a] tracking-wide">{test.name}</p>
                  <p className="text-[0.65rem] text-muted-foreground mt-1 uppercase tracking-widest">{test.role}</p>
                </div>
              </StaggerItem>
            ))}
          </StaggerGroup>
