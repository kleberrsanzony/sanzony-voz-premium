...
/* ━━━ Organic Gold Curves (SVG) — Deactivated for Speed ━━━
const GoldCurves = () => (
  <>
  ...
  return (
    <section id="inicio" ref={containerRef} className="relative min-h-screen flex items-center overflow-hidden">
      ...
      <div className="absolute inset-0" style={{ background: 'hsl(0 0% 2%)' }}>
        {/* 
        1. Abyss background: Deep cinematic radials for 3D depth
        ...
        2. Gold atmospheric glow: 'Breathing' blobs for luxury aura
        <motion.div
        ...
        */}
        ...
        {/* 
        4. Gold curves decorative: Organic brand signature curves /
        <GoldCurves />
        */}
      ...
      {/* ━━━ Content ━━━ */}
      <motion.div
        className="container-site relative z-10 pt-32 md:pt-48 pb-20"
        style={{ opacity: opacityFade }}
      >
        <div className="grid md:grid-cols-12 gap-12 lg:gap-16 items-end mt-[6%]">
          ...
          <motion.div className="md:col-span-7" style={{ y: yHeadline }}>
            ...
            <motion.h1
              className="mt-10 font-display font-bold text-[2.4rem] xs:text-[2.8rem] md:text-[4rem] lg:text-[5.5rem] leading-[0.95] tracking-[-0.03em] will-change-transform"
            >
              <div className="overflow-hidden flex flex-wrap gap-x-[0.3em]">
                {"Sua marca não".split(" ").map((word, i) => (
                  <motion.span
                    key={i}
              ...
              <div className="overflow-hidden flex flex-wrap gap-x-[0.3em]">
                {"precisa de uma voz.".split(" ").map((word, i) => (
                  <motion.span
                    key={i}
                    className="inline-block will-change-transform"
                    initial={{ y: "110%", opacity: 0 }}
                    animate={{ y: 0, opacity: 1 }}
                    transition={{ duration: 0.8, delay: 1.2 + i * 0.1, ease: [0.16, 1, 0.3, 1] }}
                  >
                    {word}
                  </motion.span>
                ))}
              ...
              <motion.span
                className="block text-gold-shimmer mt-2 will-change-transform"
            ...
            <motion.p
              className="mt-8 md:mt-10 text-secondary-foreground text-sm md:text-base leading-[1.8] md:leading-[2] max-w-sm md:max-w-md"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            <motion.div
              className="mt-10 md:mt-12 flex flex-col sm:flex-row gap-5"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 1, delay: 2.2, ease: [0.16, 1, 0.3, 1] }}
            >
              <a href="#demos" className="btn-primary w-full sm:w-fit" data-cursor="grow"><span>Ouvir Demos</span></a>
              <a href="#contato" className="btn-outline w-full sm:w-fit" data-cursor="grow"><span>Solicitar Orçamento</span></a>
            </motion.div>
          </motion.div>
              ...
              <div className="flex flex-wrap gap-2 mb-7">
                {heroCategories.map((c) => (
                  <button
                    ...
                    className={`px-4 py-1.5 text-[0.5rem] uppercase tracking-[3px] rounded-sm transition-all duration-700 cursor-pointer ${activeTab === c ? 'text-gold-static font-semibold' : 'text-label hover:text-silver'
                      }`}
                    style={
                      activeTab === c
        ...
        >
          <div className="scroll-indicator">
            ...
            <span className="text-[0.60rem] uppercase tracking-[5px] text-gold-muted" style={{ color: '#7a5c2e' }}>explorar</span>
          </div>
        </motion.div>
      </motion.div>
    </section>
