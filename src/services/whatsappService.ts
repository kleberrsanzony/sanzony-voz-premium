...
const INSTANCE = process.env.NEXT_PUBLIC_EVOLUTION_INSTANCE_NAME;
...
export const sendPtt = async (payload: DeliveryPayload): Promise<SendResult> => {
  ...
  const jid = normalizeJID(payload.whatsapp);
  const SYB_URL = (process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://eazwewzslriqzzvjwpjh.supabase.co').replace(/\/$/, '');
