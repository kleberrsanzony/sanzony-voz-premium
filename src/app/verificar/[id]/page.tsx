...
function ValidCard({ data }: { data: CertificateResult }) {
  ...
  return (
    <div className="relative border border-emerald-500/25 rounded-lg overflow-hidden bg-emerald-500/[0.03] animate-in fade-in slide-in-from-bottom-4 duration-700">
      {/* top accent bar */}
      ...
      <div className="p-8 space-y-7">
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">Emitido em {date}</p>
          </div>
        </div>
        ...
        <dl className="grid sm:grid-cols-2 gap-x-8 gap-y-5">
          <div>
            <dt className="text-[0.6rem] uppercase tracking-widest text-muted-foreground mb-1">Cliente</dt>
            <dd className="font-semibold text-white text-sm">{data.nome}</dd>
          </div>
          {data.empresa && (
            <div>
              <dt className="text-[0.6rem] uppercase tracking-widest text-muted-foreground mb-1">Empresa</dt>
              <dd className="font-semibold text-white text-sm">{data.empresa}</dd>
            </div>
          )}
          <div>
            <dt className="text-[0.6rem] uppercase tracking-widest text-muted-foreground mb-1">Tipo de Locução</dt>
            <dd className="font-semibold text-white text-sm">{data.tipo_locucao}</dd>
          </div>
          <div>
            <dt className="text-[0.6rem] uppercase tracking-widest text-muted-foreground mb-1">Status</dt>
...
function InvalidCard() {
  return (
    <div className="relative border border-red-500/20 rounded-lg overflow-hidden bg-red-500/[0.03] animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="p-10 flex flex-col items-center text-center gap-4">
        ...
        <div>
          <p className="font-display font-bold text-white text-lg mb-2">Certificado não encontrado</p>
          <p className="text-sm text-muted-foreground max-w-sm">
            Nenhum registro corresponde a este número. Verifique se digitou corretamente ou entre em contato com o Studio Sanzony.Voz.
          </p>
        </div>
      </div>
    </div>
...
export default function VerificarPage() {
  ...
  const performVerification = async (certNumber: string) => {
    ...
    try {
      const { data, error } = await supabase.rpc("verify_certificate", { 
        cert_number: certNumber.toUpperCase().trim() 
      });
  ...
  // Initial load from dynamic route id
  useEffect(() => {
    if (params?.id) {
      const idStr = Array.isArray(params.id) ? params.id[0] : params.id;
      setQuery(idStr);
      performVerification(idStr);
    }
  }, [params?.id]);

  const handleVerify = () => performVerification(query);
  ...
  return (
    <main className="min-h-screen bg-black flex flex-col">
      ...
      <div className="flex-1 flex items-start justify-center pt-16 pb-24">
        <div className="container-site w-full max-w-xl px-6">
          ...
          <div className="mb-10 text-center">
            ...
            <span className="section-label block mb-4">Autenticidade</span>
            <h1 className="font-display font-bold text-4xl text-white tracking-tight leading-tight mb-4">
              Verificar{" "}
              <span className="text-gold">Certificado</span>
            </h1>
            <p className="text-sm text-muted-foreground leading-relaxed">
              Confirme a originalidade do áudio produzido pela Sanzony.Voz.
            </p>
          </div>
          ...
          <div className="flex gap-3 mb-8">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Ex: SVZ-2026-ABR-001"
          ...
          {!searched && !loading && (
            <div className="flex items-center gap-3 border border-white/[0.06] rounded-md p-4 bg-white/[0.02]">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                Este sistema utiliza hash SHA-256 e assinatura digital 
                para garantir a integridade absoluta de cada projeto entregue.
              </p>
            </div>
          )}
          ...
          {loading && !searched && (
            <div className="flex flex-col items-center py-10 gap-3">
              ...
              <p className="text-xs text-muted-foreground">Consultando banco de certificados...</p>
            </div>
          )}
        </div>
      </div>
      ...
      <footer className="border-t border-white/5 py-6 text-center">
        <p className="text-[0.6rem] text-muted-foreground uppercase tracking-widest">
          © {new Date().getFullYear()} Sanzony.Voz · Autenticidade Digital
        </p>
      </footer>
    </main>
  );
